...
const ReportedUsers = () => {
  ...
  const handleDeleteComment = async (id) => {
    const response = await bookService.deleteComment(id);

    if (response.status === 200) {
      setReportList((prev) => prev.filter((item) => item.iD_Comment !== id));
    }
  };
                        ...
                        {reportList.map((person) => (
                          ...
                          >
                            ...
                            <td className="whitespace-nowrap py-4 px-6 text-right text-sm font-medium">
                              <button
                                className="mr-2 rounded-md bg-red-600 px-4 py-1 text-white hover:bg-red-700 hover:text-indigo-900"
                                onClick={() => {
                              ...
                              <button
                                ...
                                onClick={() => {
                                  handleDeleteComment(person.iD_Comment);
                                }}
                              >
                                Xóa
                              </button>
